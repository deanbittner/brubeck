  ...
  while (buffer < end) {
    ...
    if (brubeck_statsd_msg_parse(&msg, buffer, stat_end) < 0) {
      ...
      if (gh_log_all_metrics > 0)
      {
        ...
        log_splunk("sampler=statsd event=metchk bad_metric=%s stamp=%d", raw, (int)now);
        if (raw) free (raw);
      }
    } else {
      ...
      if (metric != NULL)
      {
        ...
        if (gh_log_all_metrics > 0)
        {
          ...
          if (gh_log_all_regex != NULL && filter == NULL)
          {
            ...
            {
              log_splunk("sampler=statsd event=metchk badregex=%s", gh_log_all_regex);
            }
            else
            {
              regex_good = 1;
          ...
          log_splunk("sampler=statsd event=metchk metric=%s value=%.1f type=%s stamp=%d", key, value, type, (int)now);
        NOMATCH:
          if (key) free(key);
        }
      }
